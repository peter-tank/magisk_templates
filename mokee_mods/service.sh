...
wait_boot_complete && {
[ -f "$MODPATH/mokee_settings.sh" ] && . $MODPATH/mokee_settings.sh;
logf_print 0 "- waiting system-server crash...";
$TEST || $BINPATH/svc "system-server" "wait-for-crash" >/dev/null;
_rt=$?;
if test "$_rt" -eq 0; then
  $TEST && logf_print "$_rt" "- crash wait skiped for on device script testing." || logf_print "$_rt" "- system shutdown gracefully.";
else
  touch disable;
